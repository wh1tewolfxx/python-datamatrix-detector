    ...
    def detect(self, target_image):
        ...
        for contour in contours:
            ...
            if width != 0 and height != 0:
                ...
                if (self.max_area * self.max_area_lower_limit) < target_area < (
                        self.max_area * self.max_area_upper_limit) and (
                        aspect_ratio * self.ratio_lower_limit) < self.ratio < (aspect_ratio * self.ratio_upper_limit):
                    # Draw the bounding box using the minimum area rectangle
                    ...
                    d = DatamatrixDetector.Detection(roi=roi, rect=rect)
                    detections.append(d)
        return detections